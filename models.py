...
class DailyMood(Base):
    ...
    def __repr__(self):
        return f"<DailyMood(date={self.date}, happiness={self.happiness}, energy={self.energy}, stressed={self.stressed}, friends_family_time={self.friends_family_time}, notes={self.notes})>"
   
class DailyAutomatedData(Base):
    __tablename__ = "daily_automated_data"
    date = Column(Date, primary_key=True)
    weather_code = Column(String, nullable=True)
    def __repr__(self):
        return f"<DailyAutomatedData(date={self.date}, weather_code={self.weather_code})>"